...
その2つをタブで行き来できる、中〜上級者向けのコードツールです。

**→ [使ってみる](https://kissou3030.github.io/guitar-chord-lab/)**

![オープンCを押さえた状態。押さえた音の度数が色で示され、下に判定結果が並ぶ](screenshot.png)
...
「指板から調べる」と「コード名から探す」はタブで分かれています。以前は1本のスクロールに縦積みでしたが、
指板をいじると判定結果のすぐ下に無関係なコードの押さえ方が並んでしまい、同時に見えることが害になっていました。

ただし切り離してはいません。判定結果の候補をタップすれば検索タブへ、カードをタップすれば指板タブへ、
操作が自動でタブを越えます。離れたタブの検索語・フィルタ・運指・スクロール位置はそのまま残るので、
行き来しても続きから作業できます。

指板は「指板から調べる」タブにひとつだけ存在し、それが「今の状態」を表す唯一の場所です。
